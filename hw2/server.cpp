...
void Server::ServerListen() {
    ...
    char clientIP4[INET_ADDRSTRLEN];
    memset(clientIP4, 0, sizeof(clientIP4));

    if (!inet_ntop(clientAddr.sin_family, &(clientAddr.sin_addr), clientIP4, sizeof(clientIP4)))
    {
        perror("inet_ntop");
    }
    else
    {
        cout<<"A client "<<clientIP4<<" has connected via port "<<ntohs(clientAddr.sin_port)<<" using SOCK_STREAM (TCP)"<<endl;
    }
}
...
void Server::ReceiveFile(string fileName, int fileSize)
{
    ...
    cout<<"File "<<"receive_"<<fileName<<" Store Success"<<endl;
...
void Server::ClinetClose()
{
    ...
    {
        cout<<"The client "<<clientIP4<<" with port "<<ntohs(clientAddr.sin_port)<<" has terminated the connection."<<endl;
    }
